...
public class Hibernate {
	...
	public Hibernate(Connection connection)
	{
		this.connection = connection;
	}
	
	public int suspendVM (int ID) {
		...
		try{
            ...
            if(vm.isActive() == 1){
				...
				System.out.println("Dominio inativo");	
	...
	public int suspendVMbyName (String name) {
...
public int hibernateVMListByName (ArrayList<String> listVMS) {
		
		conn = connection.getConnection();
		
		int size = listVMS.size();
		
		for(int i = 0 ; i < size ; i++) //Runs the ArrayList with the VM's names 
		{
			try{
